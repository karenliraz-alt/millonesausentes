export default async function handler(req, res) {
  ...
  try {
    ...
    const prompt = `A realistic and moving close-up photograph taken at Plaza de la Ciudadania in Santiago, Chile, during a sunny day. In the extreme foreground, resting on the gray paved ground, is a single ${color} ${shoe}. Resting against the shoe is a square white card with clearly printed block letter text in dark ink, easy to read: "${nombre} — ausente por fibromialgia". The word fibromialgia must be spelled exactly and legibly. Camera at ground level, focused sharply on the shoe and card. Behind it, hundreds of diverse pairs of empty shoes in rows, varied colors and styles. In the background, blurred bokeh, the facade of La Moneda Palace with Chilean flag flying from the rooftop. Clear bright blue sky.`;
